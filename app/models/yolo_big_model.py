...
class YOLOBigModel(FingerDetectorBaseModel):
    def __init__(self):
        super().__init__()
        ...
        try:
            # Загружаем модель YOLO big
            self.model = YOLO(model_path)
            print(f"YOLO Big model loaded from: {model_path}")
        except Exception as e:
            print(f"Error loading YOLO Big model: {e}")
            self.model = None
        
    def get_finger(self, frame) -> Tuple[float, float]:
        """
        Определяет положение указательного пальца с помощью YOLO big модели.
        """
        if self.model is None:
            print("YOLO Big model not initialized")
            return 0.0, 0.0
        ...
        try:
            ...
            print(f"Error in YOLO big model: {e}")
